...
import type { Bytes } from '../../types/bytes';
...
export const testBit = (bitMap: Bytes, n: number): boolean => {
  return (bitMap[parseInt((n / 8).toString(), 10)] & (1 << (n % 8))) !== 0;
};

export const testBitBigEndian = (bitMap: Bytes, n: number): boolean => {
  return (bitMap[bitMap.length - parseInt(`${n / 8}`, 10) - 1] & (1 << (n % 8))) !== 0;
};

// SetBitBigEndian sets the bit n in the bitmap to 1, in Big Endian.
export const setBitBigEndian = (bitMap: Bytes, n: number): void => {
  bitMap[bitMap.length - parseInt(`${n / 8}`, 10) - 1] |= 1 << (n % 8);
};
...
export const bytes2Hex = (u: Bytes): string => {
  ...
  let j = 0;
  u.forEach((v) => {
    arr[j] = hexTable[parseInt((v >> 4).toString(10), 10)];
    arr[j + 1] = hexTable[parseInt((v & 15).toString(10), 10)];
    j += 2;
  });
