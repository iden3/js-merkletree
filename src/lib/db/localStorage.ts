  ...
  async get(k: Bytes): Promise<Node | undefined> {
    ...
    switch (obj.type){
      ...
      case NODE_TYPE_MIDDLE:
        ...
        const cL = new Hash()
        cL.bytes =  Uint8Array.from( Object.keys(childL.bytes).map((k)=>{
          return childL.bytes[k]
        }))

        const cR = new Hash()
        cR.bytes =  Uint8Array.from( Object.keys(childR.bytes).map((k)=>{
      ...
      case NODE_TYPE_LEAF:
        ...
        const k = (new Hash())
        k.bytes =  Uint8Array.from( Object.keys(entry[0].bytes).map((k)=>{
          return entry[0].bytes[k]
        }))

        const v =(new Hash())
        v.bytes =  Uint8Array.from( Object.keys(entry[1].bytes).map((k)=>{
