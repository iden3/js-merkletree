...
for (let index = 0; index < storages.length; index++) {
  describe(`full test of the SMT library: ${storages[index].toString()}`, () => {
    ...
    it("test new merkle tree", async () => {
      ...
      expect(
        await verifyProof(await mt.root(), proof, BigInt("33"), BigInt("44")),
      ).toEqual(true);

      expect(
        await verifyProof(await mt.root(), proof, BigInt("33"), BigInt("45")),
      ).toEqual(false);
    });
    ...
    it("test tree with one node", async () => {
      const sto = getTreeStorage();
      const mt = new Merkletree(sto, true, 10);
      expect(bytesEqual((await mt.root()).value, ZERO_HASH.value)).toEqual(
        true,
      );
    ...
    it("test generate and verify proof 128", async () => {
      ...
      const verRes = await verifyProof(
        await mt.root(),
        proof,
        BigInt("42"),
        BigInt("0"),
    ...
    it("test and verify proof cases", async () => {
      ...
      expect(
        await verifyProof(await mt.root(), proof, BigInt("4"), BigInt("0")),
      ).toEqual(true);
      ...
      expect(
        await verifyProof(await mt.root(), proof, BigInt("12"), BigInt("0")),
      ).toEqual(true);
      expect(bytes2Hex(proof.bytes())).toEqual(
      ...
      expect(proof.nodeAux).toBeDefined();
      expect(
        await verifyProof(await mt.root(), proof, BigInt("10"), BigInt("0")),
      ).toEqual(true);
      expect(bytes2Hex(proof.bytes())).toEqual(
    ...
    it("test and verify proof false", async () => {
      ...
      expect(proof.existence).toEqual(true);
      expect(
        await verifyProof(await mt.root(), proof, BigInt("5"), BigInt("5")),
      ).toEqual(false);
    ...
    it("test delete leaf near middle node. Right branch", async () => {
      ...
      for (const k of keys) {
        ...
        compareSiblings(
          expectedSiblingsNonExist[k.toString()],
          existProof.proof,
        );
      }
    });
    ...
    it("test delete leaf near middle node. Left branch deep", async () => {
      ...
      const expectedSiblingsNonExist: { [id: string]: bigint[] } = {
        "4": [
          0n,
          0n,
          999617652929602377745081623447845927693004638040169919261337791961364573823n,
        ],
        "8": [],
        "16": [],
      };
    ...
    it("test up nodes to two levels. Left branch", async () => {
      ...
      compareSiblings(
        [
          0n,
          849831128489032619062850458217693666094013083866167024127442191257793527951n,
        ],
        proof.proof,
      );
    });
    ...
    it("proof stringify (old format for node aux)", async () => {
      ...
      expect(proof.existence).toEqual(false);
      expect(JSON.stringify(proof.nodeAux)).toEqual(
        JSON.stringify(proofFromJSON.nodeAux),
      );
    });
    ...
    it("should deserialize Old Hash properly", async () => {
      ...
      expect(JSON.stringify(hash)).toEqual(
        JSON.stringify(hashFromOldStr.bigInt().toString()),
      );
    ...
    it("calculate depth for mtp", async () => {
      ...
      const { proof }: { proof: Proof } = await mt.generateProof(
        BigInt("11"),
        await mt.root(),
      );
